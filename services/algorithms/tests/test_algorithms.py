...
class TestClusteringAlgorithm(unittest.TestCase):
    ...
    def test_nearby_reports_clustered_as_duplicates(self):
        ...
        result = cluster_reports(reports, epsilon_meters=100.0, min_points=2)
    ...
    def test_distant_reports_remain_unclustered_noise(self):
        ...
        result = cluster_reports(reports, epsilon_meters=100.0, min_points=2)
        self.assertEqual(len(result), 2)
        # Both must be marked as noise (-1) and not duplicates
        self.assertEqual(result[0]["clusterId"], -1)
        self.assertEqual(result[1]["clusterId"], -1)
        self.assertFalse(result[0]["isDuplicate"])
        self.assertFalse(result[1]["isDuplicate"])
    ...
    def test_clustering_execution_speed_under_100ms(self):
        ...
        cluster_reports(reports, epsilon_meters=100.0, min_points=2)
